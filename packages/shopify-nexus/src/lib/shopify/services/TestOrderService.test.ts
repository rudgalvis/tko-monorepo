...
describe.sequential('Should setup test product and perform test order', () => {
    ...
    test('Should create order on newly created product', { timeout: 15000 }, async () => {
        ...
        const r1 = await testOrderService.createOrder([
            			{
            				quantity: 1,
            				variantId: generateVariantGid(53994771480924),
            				productId: generateProductGid(14892279038300),
...
//            },
        ...
        console.log(
            `Order created. Shopify should hit ${PUBLIC_NEXUS_BASE_URL} webhook in a moment.`
        )
    })
})
