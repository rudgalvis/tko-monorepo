...
export class ProductService {
	...
	async disableSellOutOfStock({ variantId }: { variantId: string }) {
		return this.productRepository.disableSellOutOfStock({ variantGid: variantId })
	}

	async getFinalVariantPrice(countryCode: string, variantId: number, flickCall = false): Promise<FinalVariantPrice> {
        const {
            price: { amount: price },
            compareAtPrice,
            availableForSale,
        ...
        const discount = await this.storefrontApi.getAutomaticDiscountForVariant(
            countryCode,
            variantId
        )

        if (!availableForSale && !flickCall) {
            const variantGid = generateVariantGid(variantId)

            console.log('Item is not available for sale, enabling selling out of stock for a moment')

            // Make it available for sale by enabling selling out of stock
            await this.productRepository.enableSellOutOfStock({variantGid})

            // Repeat this call
            const result = await this.getFinalVariantPrice(countryCode, variantId, true)

            console.log('Got the result. Disabling seeling out of stock')

            // Disable selling out of stock
            await this.productRepository.disableSellOutOfStock({variantGid})

            return result

        } else if(!availableForSale) {
            throw Error('Variant is not available for sale')
        }
